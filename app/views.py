...
def signout(request):
    # each store's data is wiped on logout rather than persisted long-term,
    # so a clean slate is waiting the next time someone signs in
...
# ---- JSON APIs consumed by the dashboard's charts/widgets ----
...
def run_full_inventory_ai_engine(request):
    # this is the "do everything" endpoint - runs the forecast, then feeds
    # it straight into the stock recommendation engine in one go
    store = request.user.storeowneres
