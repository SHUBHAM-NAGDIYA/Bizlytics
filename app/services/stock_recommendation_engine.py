...
# extra cushion on top of the raw forecast so a store isn't ordering exactly
# down to the unit - 20% headroom for demand spikes
...
def generate_stock_recommendations(forecast_dict, store):
    ...
    with transaction.atomic():
        # old recommendations go stale the moment a new forecast is run,
        # so clear them out before writing the fresh batch
        ...
        for product_id, forecast_data in forecast_dict.items():
            ...
            StockRecommendation.objects.create(
                ...
                ForecastSevendays=forecast_7_days,
